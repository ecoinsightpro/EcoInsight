...
function EcoNews({t, lang, go}) {
  ...
  const [region, setRegion] = React.useState('internacional');
    ...
    if(window.fetchEcoNews){
      ...
      Promise.race([window.fetchEcoNews(lang), deadline]).then(arr=>{
        if(arr && arr!=='__timeout' && arr.length>=4){
          ...
          try{
            localStorage.setItem('ei_news_sync', String(now));
            localStorage.setItem(CACHE_KEY, JSON.stringify({lang, at:now, items:arr}));
          }catch(e){}
        } else if(!silent){
          // curated fallback: stamp the sync now so the masthead never shows a stale age
          setFeedState('sample');
          const now=Date.now(); setLastSync(now);
          try{ localStorage.setItem('ei_news_sync', String(now)); }catch(e){}
        }
      }).catch(()=>{ if(!silent){ setFeedState('sample'); const now=Date.now(); setLastSync(now); try{ localStorage.setItem('ei_news_sync', String(now)); }catch(e){} } });
    } else if(!silent){
      setFeedState('sample');
      const now=Date.now(); setLastSync(now);
      try{ localStorage.setItem('ei_news_sync', String(now)); }catch(e){}
  ...
  // ── ONE FETCH PER DAY ──
  // On open, reuse today's cached edition; only hit the network when the cache is
  // from another day (or missing), so the feed refreshes once a day, not per visit.
  React.useEffect(()=>{
    let cached = null;
    try{ cached = JSON.parse(localStorage.getItem(CACHE_KEY)||'null'); }catch(e){}
    if(cached && cached.lang===lang && Array.isArray(cached.items) && cached.items.length>=4 && sameDay(cached.at, Date.now())){
      setLive(cached.items); setFeedState('live'); setLastSync(cached.at);
      return;
    }
    load(false);
  }, [lang, load]);
  ...
  // ── DAILY AUTO-UPDATE ──
  // For a page left open: re-fetch when the calendar day rolls over (or 24h elapse),
  // and re-check on tab refocus, so it still updates exactly once a day.
  React.useEffect(()=>{
    const ONE_DAY = 24*3600*1000;
    const check = () => {
      const last = parseInt(localStorage.getItem('ei_news_sync')||'0',10)||0;
      const now = Date.now();
      if(now-last > ONE_DAY || !sameDay(last, now)) load(true);
    };
  ...
  // keep the layout always full: top up with the rest of the feed (matches first).
  // Only a no-match search stays empty, so search still communicates "no results".
  const need = Math.min(7, SRC.length);
  let display = filtered;
  if(filtered.length>0 && filtered.length<need){
    const have = new Set(filtered.map(x=>x.id));
    display = [...filtered, ...SRC.filter(x=>!have.has(x.id)).slice(0, need-filtered.length)];
  }
  const featured = display[0];
  const sidebar = display.slice(1,4);
  const grid = display.slice(4,7);
  ...
  return (
    <section id="noticias" style={{background:t.bg,padding:'96px 0',borderTop:`1px solid ${t.border}`,scrollMarginTop:84}}>
      <div style={{maxWidth:1180,margin:'0 auto',padding:'0 32px'}}>
        <Reveal>
        {/* ═══ editorial "wire desk" masthead ═══ */}
        <div style={{border:`1px solid ${t.border}`,borderRadius:22,overflow:'hidden',marginBottom:24,background:t.card,boxShadow:`0 18px 50px rgba(0,0,0,${t.dark?.35:.06})`}}>
          {/* console top bar */}
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,padding:'12px 20px',borderBottom:`1px solid ${t.border}`,background:t.surface,flexWrap:'wrap'}}>
            <div style={{display:'inline-flex',alignItems:'center',gap:9}}>
              ...
              <span style={{color:t.critical,fontSize:11,fontWeight:800,letterSpacing:'.12em',textTransform:'uppercase',fontFamily:'Plus Jakarta Sans',whiteSpace:'nowrap'}}>{L('Actualidad ESG & Ambiental','ESG & Environmental Briefing')}</span>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
              <span style={{display:'inline-flex',alignItems:'center',gap:7,fontSize:11.5,color:feedState==='live'?t.positive:feedState==='loading'?t.warning:t.textMuted,fontFamily:'DM Mono',fontWeight:600}}>
              ...
              <span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:10.5,color:t.textFaint,fontFamily:'DM Mono'}}>
                <span style={{color:t.accent}}>↻</span>{L('Actualiza','Sync')} {syncSince} · {syncAt}
              </span>
            </div>
          </div>
          {/* headline body */}
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:26,padding:'30px 26px 26px',flexWrap:'wrap'}}>
            <div style={{minWidth:0,flex:'1 1 380px'}}>
              <h2 style={{fontSize:'clamp(32px,4vw,46px)',fontWeight:800,fontFamily:'Plus Jakarta Sans',color:t.text,lineHeight:1.06,letterSpacing:'-.03em',marginBottom:14,position:'relative',display:'inline-block'}}>
                {L('Centro de Inteligencia','Environmental')}<br/><span style={{position:'relative'}}>{L('Ambiental','Intelligence Hub')}<span style={{position:'absolute',left:0,right:0,bottom:2,height:'.34em',background:t.accent,opacity:.24,borderRadius:3,zIndex:-1}}/></span>
              </h2>
              <p style={{fontSize:16,color:t.textMuted,fontFamily:'DM Sans',lineHeight:1.62,maxWidth:520}}>{L('Monitoreo diario de ESG, clima y regulación para LATAM — cada nota con resumen IA e impacto directo para Chile y tu empresa.','Daily monitoring of ESG, climate and regulation across LATAM — every story with an AI summary and direct impact for Chile and your company.')}</p>
            </div>
            {/* intelligence metrics */}
            <div style={{display:'flex',gap:0,flexShrink:0}}>
              {[[SRC.length, L('Notas en el feed','Stories in feed')],[new Set(SRC.map(x=>x.cat)).size, L('Temas','Topics')],[2, L('Regiones','Regions')]].map(([v,l],i)=>(
                <div key={l} style={{padding:'2px 20px',borderLeft:i?`1px solid ${t.border}`:'none',textAlign:'center'}}>
                  <div style={{fontSize:30,fontWeight:800,fontFamily:'Plus Jakarta Sans',color:t.text,lineHeight:1,letterSpacing:'-.02em'}}>{v}</div>
                  <div style={{fontSize:10,color:t.textFaint,fontFamily:'Plus Jakarta Sans',fontWeight:600,textTransform:'uppercase',letterSpacing:'.07em',marginTop:6,whiteSpace:'nowrap'}}>{l}</div>
                </div>
              ))}
            </div>
          </div>
          {/* live headline ticker */}
          <div className="ei-wire" style={{display:'flex',alignItems:'stretch',borderTop:`1px solid ${t.border}`,background:t.surface}}>
            <div style={{display:'inline-flex',alignItems:'center',gap:7,padding:'0 16px',background:t.critical,color:'#fff',fontSize:10,fontWeight:800,letterSpacing:'.12em',fontFamily:'Plus Jakarta Sans',flexShrink:0}}>
              <span style={{width:6,height:6,borderRadius:'50%',background:'#fff',display:'inline-block',animation:'pulse 1.4s infinite'}}/>{L('EN VIVO','LIVE')}
            </div>
            <div style={{overflow:'hidden',flex:1,minWidth:0,display:'flex',alignItems:'center',WebkitMaskImage:`linear-gradient(90deg,transparent,#000 4%,#000 96%,transparent)`,maskImage:`linear-gradient(90deg,transparent,#000 4%,#000 96%,transparent)`}}>
              <div className="ei-ticker-track" style={{display:'inline-flex',alignItems:'center',whiteSpace:'nowrap'}}>
                {[...SRC.slice(0,10),...SRC.slice(0,10)].map((a,i)=>(
                  <button key={i} onClick={()=>go('article',a.id)} style={{display:'inline-flex',alignItems:'center',gap:8,background:'transparent',border:'none',cursor:'pointer',padding:'11px 22px',fontFamily:'DM Sans',fontSize:12.5,color:t.textMuted,transition:'color .18s'}}
                    onMouseEnter={e=>e.currentTarget.style.color=t.text} onMouseLeave={e=>e.currentTarget.style.color=t.textMuted}>
                    <span style={{width:5,height:5,borderRadius:'50%',background:cc(a),flexShrink:0}}/>{a.title[lang]}
                  </button>
                ))}
              </div>
            </div>
